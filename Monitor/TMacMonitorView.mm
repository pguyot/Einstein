...
@interface TMacMonitorView ()
{
	...
	NSAttributedString *history[HISTORY_SIZE];
	CGFloat historyOffset;
	NSInteger historyVisible;
...
- (void)drawRect:(NSRect)dirtyRect
{
	...
	textPoint = frame.origin;
	textPoint.x = 4 + (FONT_WIDTH * 21);
	textPoint.y = (FONT_HEIGHT * 6);
	...
	[NSBezierPath strokeLineFromPoint:NSMakePoint(FONT_WIDTH * 18, (FONT_HEIGHT * 9) - 3) toPoint:NSMakePoint(FONT_WIDTH * 18, frame.size.height)];

	// Horizontal, between SPSR and Tmr

	NSInteger y = frame.size.height - (FONT_HEIGHT * 18.5);

	[NSBezierPath strokeLineFromPoint:NSMakePoint(0, y) toPoint:NSMakePoint(FONT_WIDTH * 18, y)];

	// Horizontal, between registers and disassembly

	y = (FONT_HEIGHT * 9) - 4;
	
	[NSBezierPath strokeLineFromPoint:NSMakePoint(0, y) toPoint:NSMakePoint(w, y)];
...
- (void)scrollWheel:(NSEvent *)event
{
	...
	if ( dy != 0.0 )
	{
		...
		if ( historyOffset < 0.0 )
			historyOffset = 0.0;
		
		if ( historyOffset > (int)(HISTORY_SIZE - historyVisible) )
			historyOffset = (CGFloat)(HISTORY_SIZE - historyVisible);
		
		if ( prevOffset != historyOffset)
			[self setNeedsDisplay:YES];
	}
}
