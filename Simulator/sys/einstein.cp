...
namespace Simulator {
  ...
  /**
   * If Simulator calls the emulator, jumping to address 0x007ffff0 will return
   * into the Simulator native code.
   */
  JITInstructionProto(ReturnToSimulator_Stub);
  TROMPatch pReturnToSimulator(0x007ffff0, ReturnToSimulator_Stub, "ReturnToSimulator", (AnyFunctionPtr)0L);
  JITInstructionProto(ReturnToSimulator_Stub) {
    printf("## untested: Returning from emulator to Simulator\n");
    return 0;
  }
 
  void callEmulator(KUInt32 address) 
  {
    printf("## untested: calling from Simulator into emulator\n");
    KUInt32* pcPtr = &TARMProcessor::current->mCurrentRegisters[TARMProcessor::kR15];
    TMemory* theMemoryInterface = TARMProcessor::current->GetMemory();
    TARMProcessor::current->mCurrentRegisters[TARMProcessor::kR14] = 0x007ffff0; // return to Simulator
