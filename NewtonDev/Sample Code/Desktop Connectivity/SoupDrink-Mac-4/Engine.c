...
#define LongSwap 0		// don't need to swap longs on MacOS
...
#define LongSwap 4		// when reading a long with CDPipeRead, need to swap bytes.
...
CommErr SoupDrink() 
{
...
#ifndef Workaround10WinDILsBug
	CHECKDILERROR((short) CDPipeAccept(ourPipe));
#else
	// This code was needed for the 1.0 Windows DILs.  (Not for 1.0.2, or MacOS.)
	// You need to loop a little bit for the connection state to be set to 
	// kCDIL_ConnectPending.  It might not do that if there's nobody on the
	...
	if (gInputMode == kDrinkMode) {
		...
		fErr = (short) CDPipeRead(ourPipe, (void *) &gReceivedStringLen, &longLen, &eom,LongSwap,Encoding,Timeout,0,0);
		if (fErr == 0)
...
CommErr UploadNewName ()
{
	...
	CHECKDILERROR((short) FDbindSlot(entry, "cardType", (void *)&cardType, kDILInteger, sizeof(cardType), -1, NULL)) ;
