...
// #elif TARGET_OS_LINUX
	...
	static void
	penDownTimerCB(void* me)
	{
		((Fl_Newton_Screen_Widget*) me)->penDownTimer();
	}

	int
	handle(int event) override
	{
		switch (event)
		{
			case FL_PUSH:
...
TFLScreenManager::TFLScreenManager(
	...
	Boolean inScreenIsLandscape /* = true */) :
		TScreenManager(
			...
			inFullScreen,
			inScreenIsLandscape),
		mApp(inApp), mWidget(0L), mScreenWasLandscape(inScreenIsLandscape)
{
...
TFLScreenManager::UpdateScreenRect(SRect* inUpdateRect)
{
	...
	if (inUpdateRect)
	{
		...
		if (bottom >= mWidget->getRGBHeight())
			bottom = mWidget->getRGBHeight() - 1;
		if (top > bottom)
			return;
		height = bottom - top;
		...
		if (right >= mWidget->getRGBWidth())
			right = mWidget->getRGBWidth() - 1;
		if (left > right)
			return;
		width = right - left;
	} else
	...
	KUInt8* dstRowPtr = ((KUInt8*) mWidget->getRGBData())
		+ (top * dstRowBytes)
