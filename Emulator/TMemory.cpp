...
TMemory::SetEmulator(TEmulator* inEmulator)
{
	if (inEmulator)
	{
		...
		{
			mPCMCIACtrls[socketIx] = new TPCMCIAController(mLog, mEmulator, socketIx);
		}
...
/**
 ...
 \param[in] inAddress address before applying any MMU tables
...
TMemory::ReadP(PAddr inAddress, Boolean& outFault)
{
	if (!(inAddress & TMemoryConsts::kROMEndMask))
	{
		...
		{
			// UNALIGNED ACCESS
			KUInt32 theValue = *((KUInt32*) ((KUIntPtr) mROMImagePtr + (inAddress & 0xFFFFFFFC)));
	...
	} else if (inAddress < mRAMEnd)
	{
		...
		{
			// UNALIGNED ACCESS
			KUInt32 theValue = *((KUInt32*) ((KUIntPtr) mRAMOffset + (inAddress & 0xFFFFFFFC)));
			KUInt32 rotAmount = (inAddress & 0x3) * 8;
			return (theValue << rotAmount)
				| (theValue >> (32 - rotAmount));
		} else
...
TMemory::ComputeSerialNumber(const KUInt32 inNewtonID[2])
{
	...
	mSerialNumber[0] = (theCRC << 24)
		| (inNewtonID[0] << 8)
		| (inNewtonID[1] >> 24);
	mSerialNumber[1] = (inNewtonID[1] << 8)
		| 0x01;
}
...
Boolean
TMemory::AddWatchpoint(VAddr inAddr, KUInt8 inMode)
{
...
Boolean
TMemory::ClearWatchpoint(VAddr inAddr)
{
	// find the wp
	for (KUInt32 i = 0; i < mWPCount; i++)
	{
...
Boolean
TMemory::GetWatchpoint(int inIndex, VAddr& outAddress, KUInt8& outMode)
{
	if (inIndex >= (int) mWPCount)
		return true;
