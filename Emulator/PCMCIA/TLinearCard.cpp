...
// -------------------------------------------------------------------------- //
int
TLinearCard::Init(TPCMCIAController* inController)
{
 ...
 */
KSInt32
TLinearCard::ComposeImageFile(const char* inOutFilename, const char* inDataFilename, const char* inCISFilename, const char* name)
{
	KSInt32 err = 0;
	FILE* fOut = nullptr;
	FILE* fData = nullptr;
	...
	if (!fData)
	{
		err = kErrCantOpenDataFile;
		goto cleanup;
	}
	fseek(fData, 0, SEEK_END); // simple way to find file size
	imageInfo.pDataSize = (int) ftell(fData);
	fseek(fData, 0, SEEK_SET);
	if (imageInfo.pDataSize == 0)
	{
		err = kErrCorruptDataFile;
		goto cleanup;
	}
	if (imageInfo.pDataSize > 128 * 1024 * 1024)
	{
		err = kErrDataFileTooBig;
		goto cleanup;
	}
	...
	if (fread(dataBuffer, imageInfo.pDataSize, 1, fData) != 1)
	{
		err = kErrCorruptDataFile;
		goto cleanup;
	}
	if (fwrite(dataBuffer, imageInfo.pDataSize, 1, fOut) != 1)
	{
		err = kErrIncompleteImageFile;
		goto cleanup;
	}
	...
	if (fread(cisBuffer, rawCISSize, 1, fCIS) != 1)
	{
		err = kErrCorruptCISFile;
		goto cleanup;
	}
	...
	if (fwrite(cisBuffer, imageInfo.pCISSize, 1, fOut) != 1)
	{
		err = kErrIncompleteImageFile;
		goto cleanup;
	}
...
KSInt32
TLinearCard::CreateImageFile(const char* inName, const char* inImageFilename, KUInt32 inSizeMB)
{
	KSInt32 err = 0;
	FILE* fOut = nullptr;
	KUInt8* dataBuffer = nullptr;
...
void
TLinearCard::ImageInfo::flipByteOrder()
{
...
int
TLinearCard::ImageInfo::read(FILE* f)
{
...
int
TLinearCard::ImageInfo::write(FILE* f)
{
	flipByteOrder();
...
void
TLinearCard::MarkPageDirty(KUInt32 inAddress)
{
	std::lock_guard<std::mutex> guard(mMutex);
...
void
TLinearCard::FlushDirtyPagesLater()
{
...
void
TLinearCard::AbortFlushDirtyPages()
{
