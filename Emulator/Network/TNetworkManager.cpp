...
void
TNetworkManager::LogBuffer(KUInt8* data, ssize_t size)
{
	if (mLog)
	{
...
void
TNetworkManager::LogARPPacket(KUInt8* data, ssize_t size)
{
	(void) data;
	(void) size;
	if (mLog)
...
void
TNetworkManager::LogIPv4Packet(KUInt8* d, ssize_t n)
{
	if (mLog)
	{
		...
		mLog->FLogLine("    [16] Total Length: %d (%d)", (d[16] << 8) | d[17], (int) (n - 14)); // minus 14 bytes for Ethernet header
...
void
TNetworkManager::LogTCPPacket(KUInt8* d, ssize_t n)
{
	if (mLog)
	{
...
void
TNetworkManager::LogUDPPacket(KUInt8* d, ssize_t n)
{
	if (mLog)
	{
...
void
TNetworkManager::LogPacket(KUInt8* d, ssize_t n)
{
	if (mLog)
	{
		...
		mLog->FLogLine("  [12] Type:   0x%04x", (int) t);
		switch (t)
		{
			case 0x0800:
				LogIPv4Packet(d, n);
			...
			case 0x0806:
				...
				break;
			case 0x809b:
				mLog->LogLine("  **** unsupported: Apple Talk ****");
				break;
			case 0x86dd:
				mLog->LogLine("  **** unsupported: IPv6 ****");
				break;
			default:
				mLog->FLogLine("  **** unsupported type: 0x%04x ****", (int) t);
				break;
		}
	}
}
...
KUInt16
TNetworkManager::GetUDPChecksum(KUInt8* d, ssize_t n, Boolean set)
{
	KUInt32 s = 0;
	KUInt16 UDPLength = n - 34;
	ssize_t i;
	// UDP Pseudo Header:
...
KUInt16
TNetworkManager::GetTCPChecksum(KUInt8* d, ssize_t n, Boolean set)
{
	KUInt32 s = 0;
	KUInt16 TCPLength = n - 34;
	ssize_t i;
	// TCP Pseudo Header:
	s = s + (d[26] << 8) + d[27] + (d[28] << 8) + d[29]; // src IP
	s = s + (d[30] << 8) + d[31] + (d[32] << 8) + d[33]; // dst IP
