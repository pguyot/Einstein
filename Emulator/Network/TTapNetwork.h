...
/// 2. setup the NAT on the Mac with:
/// 2a. $ sudo /usr/sbin/sysctl -w net.inet.ip.forwarding=1
/// 2b. setup the firewall rule for NAT:
...
/// 3. start Einstein with TTapNetwork as the network manager.
/// 4. once Einstein is started (i.e. you clicked the start button), configure
/// the tap0 interface (on the Mac side) with:
/// $ sudo ifconfig tap0 10.0.1.1
/// (a totally different IP than your Mac's main IP).
/// 4. insert the NE2K card from the Platform menu.
/// 5. configure the Newton as follows:
